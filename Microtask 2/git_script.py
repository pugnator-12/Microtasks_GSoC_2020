# python3 git_script.py -backend gitlab -repo silentprogrammers/vacation_system -t z2SDNdQ2P3fD_XeFM_fp -f gitlab_mt2.output
# python3 git_script.py -backend github -repo Sentdex/sentdebot -t be4acc056dbc9abbd0e1864e14b5b84c107eb707 -f github_op.output
...
class PercevalBackends:
    ...
    def __get_gitlab_data(self):
        gitlabObj = GitLab(owner=self.owner, repository=self.repo, api_token=self.token)
        return gitlabObj

    def __get_github_data(self):
        gitlabObj = GitHub(owner=self.owner, repository=self.repo, api_token=[self.token])
        return gitlabObj
...
parser = argparse.ArgumentParser(description="perceval for github and gitlab backends")
parser.add_argument("-backend",choices=['github','gitlab'],help="enter backend(gitlab/github)", dest="backend",type=str,required=True)
parser.add_argument("-repo",help="enter repo (owner/repo)",dest="repo",type=str,required=True)
parser.add_argument("-t",help="github/gitlab personal access token",dest="token",type=str,required=True)
